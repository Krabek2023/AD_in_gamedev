...
- Здоровье(hp), ее роль в игре отображать насколько игрок близок к поражению,чем ближе к 0 тем ближе к концу игры.
  ...
  - если игрок получил урон, то значение становиться меньше
  - если игрок во время прокачки выбрал соотвествующие навыки, то они восполняют и/или увеличивают здоровье
  Диапазон допустимых значений переменной, от 0 и до установленного лимита.
...
Здоровье игрока меняется в случае получения урона, это значит, что изменение этой переменной зависит от игрока и противников. Так как есть множество вариантов изменения этого параметра,я взял ситуацию когда игрок просто получает урон в течении какого-либо промежутка времени.
- В нашей ситуации график изменения значений здоровья выглядит как прямая Hp= t*(-10) + 30 (В других случаях график может меняться):
...
- Для реализации данного задания я выделил правильно по которым описывается динамика изменения здоровья:
    - Если хп выше или равно лемиту (в моем случае hp >= 30), то будет "хороший" звук
    - Если хп около среднего значения (в моем случае 30 > hp >= 20), то "средний" звук
    - Иначе будет "плохой" звук
- Для отображения этой динамики в Unity я добавил скрипт на камеру:
```C#
